...
VS Code and Codex would use CIMD, but CIMD requires the authorization server to fetch each client's metadata document, and `claude.ai` and `chatgpt.com` are unreachable from the Hong Kong host. CIMD therefore stays unmounted. DCR remains the supported dynamic path for all documented clients. Mounting it would not help Claude Code anyway: its CIMD path is broken against this issuer, because its document registers `http://localhost/callback` while the authorize request carries `http://localhost:<random>/callback`, and Better Auth's matcher grants RFC 8252 port variance to `127.0.0.1`/`[::1]` only, so every CIMD authorize fails with `invalid_redirect` (recorded beside the plugin list in `services/auth/src/auth.ts`). CIMD is future work with two triggers: a client we use drops DCR after its deprecation window, or the auth container gains an egress path to those hosts. Re-enabling then also requires the matcher to accept `localhost` port variance. When that happens, re-add `@better-auth/cimd`, mount it beside DCR with the Node fetch guard, cache metadata documents and serve them stale on fetch failure, and verify VS Code and Codex sign in over CIMD while the other clients still use DCR.
