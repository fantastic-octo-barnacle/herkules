...
def replace(path, before, after):
    ...
    s = p.read_text()
    if s.count(before) != 1:
        raise RuntimeError("New API patch drift: " + path)
...
replace(
    ...
    "\t\t\tif !HerkulesPlanMatches(plan, modelName) { continue }\n\t\t\tif err := maybeResetUserSubscriptionWithPlanTx(tx, &sub, plan, now); err != nil {",
)
replace(
...
for name in [
    ...
    p = root / "controller/subscription.go"
    s = p.read_text()
    ...
    if part.count(gate) != 1:
        raise RuntimeError("Compliance patch drift: " + name)
    ...
    if part.count(marker) != 1:
        raise RuntimeError("Plan resolution patch drift: " + name)
    part = part.replace(marker, check + marker, 1)
    s = s[:start] + part + s[end:]
    p.write_text(s)
for name in ["herkules_pools.go", "herkules_pools_test.go"]:
    (root / "model" / name).write_text(
        (pathlib.Path(__file__).parent / name).read_text()
    )
replace(
    "service/billing_session.go",
    "model.UserActiveSubscriptionsAllowWalletOverflow(relayInfo.UserId)",
    "model.HerkulesWalletOverflow(relayInfo.UserId, relayInfo.GetBillingModelName())",
)
replace(
    "controller/misc.go",
